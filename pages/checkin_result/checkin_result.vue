	...
	<view class="page">
		...
		<view class="result-container">
			...
			<view class="right">
				...
				<view class="row">
					<text class="checkin-time">上班时间 ({{ checkinTime }})</text>
		...
		<view class="checkin-report">
			...
			<view class="calender-container">
				<view class="calender" v-for="one in weekCheckin" :key="one">
					...
					<image src="../../static/icon-10" mode="widthFix" class="calender-icon" v-if="one.type=='节假日'"></image>
					<text class="day">{{one.day}}</text>
					<text class="result green" v-if="one.status=='正常'"> {{one.status}}</text>
					<text class="result yello" v-if="one.status=='迟到'"> {{one.status}}</text>
